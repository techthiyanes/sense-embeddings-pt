...
class WordEmbeddingsKeyedVectors(BaseKeyedVectors):
    ...
    def set_part_of_speech(self, word, parser):
        if parser == 'nlpnet':
            tag = tagger.tag(str(word.lower()))[0][0][1]
            # print('word: ', word + '|' + tag)
        else:
            # Set part-of-speech tag with Spacy
            import spacy
            nlp = spacy.load('pt')
            doc = nlp(word)
            tag = doc[0].pos_
    ...
    def most_similar(self, positive=None, negative=None, topn=10, restrict_vocab=None, indexer=None):
        ...
        for word, weight in positive + negative:
            # With NLPNet
            #word = self.set_part_of_speech(word, 'nlpnet')
        ...
        for line_no, line in enumerate(utils.smart_open(questions)):
            ...
            if line.startswith(': '):
                ...
                try:
                    if case_insensitive:
                        a, b, c, expected = [word.upper() for word in line.split()]
                        a = self.set_part_of_speech(a, 'nlpnet')
                        b = self.set_part_of_speech(b, 'nlpnet')
                        c = self.set_part_of_speech(c, 'nlpnet')
                        expected = self.set_part_of_speech(expected, 'nlpnet')
                    else:
                        a, b, c, expected = [word for word in line.split()]
                        a = self.set_part_of_speech(a, 'nlpnet')
                        b = self.set_part_of_speech(b, 'nlpnet')
                    ...
                    logger.debug("Skipping invalid line #%i in %s", line_no, questions)
                    continue
                if a not in ok_vocab or b not in ok_vocab or c not in ok_vocab or expected not in ok_vocab:
                    logger.debug("Skipping line #%i with OOV words: %s", line_no, line.strip())
                    continue
