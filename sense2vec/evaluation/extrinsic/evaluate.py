...
def gensim_embedding_difference(data, field1, field2):
    ...
    distances = []
    for pair in data:
        e1 = [set_part_of_speech(i) if set_part_of_speech(i) in embeddings else 'maqueros|N' for i in pair[field1]]
        e2 = [set_part_of_speech(i) if set_part_of_speech(i) in embeddings else 'maqueros|N' for i in pair[field2]]
