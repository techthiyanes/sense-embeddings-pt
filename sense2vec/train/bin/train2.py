...
def main(in_dir, out_loc, sg=1, negative=5, n_workers=4, window=5, size=300, min_count=10, nr_iter=5):
    ...
    for text_no, text_loc in enumerate(iter_dir(corpus.directory)):
        with io.open(text_loc, 'r', encoding='utf-8') as file_:
            print(text_loc)
            text = file_.read()
    ...
    for string in model.wv.vocab:
        vocab = model.wv.vocab[string]
        freq, idx = vocab.count, vocab.index
        if freq < min_count:
            continue
        vector = model.wv.syn0[idx]
        vector_map.borrow(string, freq, vector)

    print("saving the model to file...")
    vector_map.save('models2v_s300')


if __name__ == '__main__':
    plac.call(main)

#python3 train2.py ./corpora/corpora/corpora_pos_nlpnet/ ./models2v_s300_2/s2v_s300
#nohup python2 train2.py ./corpora/ ./models2v/s2v_s300 &> nohup_s2v.out&
